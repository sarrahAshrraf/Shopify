...
class CategoryViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        categoryViewModel?.bindResultToViewController = { [weak self] in
            DispatchQueue.main.async {
                if self?.categoryViewModel.checkInternetConnectivity() == true {
    ...
    @objc func segmentChanged(_ sender: UISegmentedControl) {
        updateSegmentedControlColors()
        if categoryViewModel.checkInternetConnectivity() {
            fetchCategoryData()
    ...
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        if isFiltered {
            ...
            return categoryViewModel.result.count
        }
    }
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        let product: Product
        if isFiltered {
            product = categoryViewModel.filteredProducts[indexPath.row]
        } else {
            product = categoryViewModel.result[indexPath.row]
        }
        
        let url = URL(string: product.image?.src ?? "")
        cell.productImage.kf.setImage(with: url, placeholder: UIImage(named: Constants.noImage), options: nil, progressBlock: nil, completionHandler: nil)
        cell.productName.text = product.title
//        cell.productPrice.text = String(format: "%.2f \(currencySymbol)", product.variants?[0].price * currencyRate)
//        cell.p = product.id
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if let detailsViewController = storyboard.instantiateViewController(withIdentifier: "ProductDetailsViewController") as? ProductDetailsViewController {
            ...
            detailsViewController.currencySymbol = currencySymbol
            detailsViewController.currencyRate = currencyRate
            detailsViewController.viewModel = productDetailsViewModel
            detailsViewController.favoritesViewModel = favoritesViewModel
...
//        self.backgroundColor = selected ? UIColor.systemBlue : UIColor.white
//        self.setTitleColor(selected ? UIColor.white : UIColor.black, for: .normal)
//    }
//}
